...
class SimulatorOptions(LauncherOptions):
    ...
    def __verify_namespaces_params(self) -> None:
        ...
        for namespace in SimulatorOptions.NAMESPACES:
            ...
            serialized_params = self.params[namespace]

            for param in serialized_params:
                if param not in namespace_params:
                    raise ValueError(f"Parameter {param} in serialized params not in namespace params")

            for param in namespace_params:
                if param not in serialized_params:
                    raise ValueError(f"Parameter {param} in namespace params not in serialized params")
    ...
    def adapt_string(self, namespace: str, option: str) -> str:
        ...
        value = self.diff_params[namespace][option]
        if type(value) is str:
            return f" {namespace}::{option}='{self.diff_params[namespace][option]}'"
        elif self.diff_params[namespace][option] is True:
            return f" {namespace}::{option}=true"
        elif self.diff_params[namespace][option] is False:
    ...
    def serialize(self) -> str:
        ...
        if self.params['include'] != "":
            return reduce(lambda serialized, option: serialized + f" {option}={self.params[option]}",
    ...
    def __serialize_namespace(self, namespace: str) -> str:
        ...
        serialized = ''
        if namespace in self.diff_params:
            serialized = reduce(lambda joined, option: joined + self.adapt_string(namespace, option),
                                self.diff_params[namespace],
                                "")
        return serialized

    def set_synch_mode(self, synch_mode: bool) -> None:
        self.params['server']['synch_mode'] = synch_mode
...
class WindowOptions(LauncherOptions):
    ...
    def _verify_params(self) -> None:
        expected_params = list(WindowOptions.NAMESPACES.keys())
    ...
    def adapt_string(self, namespace: str, option: str) -> str:
        ...
        if type(value) is (int or float):
            formatted = f" --{option}={value}"
        elif type(value) is str:
            if value == "on":
                formatted = f" --{option}=on"
            elif value == "off":
                formatted = f" --{option}=off"
            else:
                formatted = f" --{option}=\"{value}\""
        elif value:
            formatted = f" --{option}=true"
        elif not value:
            formatted = f" --{option}=false"
    ...
    def set_port(self, port: int) -> None:
        self.params['monitor_client']['port'] = port


class PlayerOptions(LauncherOptions):
    """ Class wraps generic player launching options. """
    ...
    def _verify_params(self) -> None:
        for field in PlayerOptions.REQUIRED_FIELDS:
            if field not in self.params:
                raise ValueError(f"Required field {field} not present in PlayerOptions")
        if type(self.params) is not dict:
            raise ValueError("Expected dictionary in field 'options' of PlayerOptions")

    def serialize(self) -> str:
        serialized_options = self.__serialize_options()
        academy_extra = f"--academy --academy-module {self.params['module-name']} --academy-mode {self.params['mode']}"
        goalie = "-g" if self.params['goalie'] else ""
        return f"{academy_extra} {goalie} {serialized_options}"

    def __serialize_options(self) -> str:
        """ Serialize player own options. """
        options = self.params['options']
        zipped_options = list(zip(options.keys(), options.values()))
        return reduce(lambda acc, option: acc + f"--{option[0]} {option[1]} ", zipped_options, '')


class CoachOptions(LauncherOptions):
    """ Class wraps generic coach launching options. """
    ...
    def __init__(self, params: dict, extra_options: dict) -> None:
        self.extra_options = extra_options
        super().__init__(params)

    def _verify_params(self) -> None:
        for field in CoachOptions.REQUIRED_FIELDS:
            if field not in self.params:
                raise ValueError(f"Required field {field} not present in CoachOptions")
        if type(self.params['options']) is not dict:
            raise ValueError("Expected dictionary in field 'options' of CoachOptions")

    def serialize(self) -> str:
        serialized_options = self.__serialize_options()
        serialized_extra_options = f"--academy-extra-options {self.__serialize_extra_options()}"
        academy_extra = f"--academy --academy-module {self.params['module-name']} --academy-mode {self.params['mode']}"
        return f"{academy_extra} {serialized_options} {serialized_extra_options}"

    def __serialize_options(self) -> str:
        """ Serialize coach own options. """
        options = self.params['options']
        zipped_options = list(zip(options.keys(), options.values()))
        return reduce(lambda acc, option: acc + f"--{option[0]} {option[1]} ", zipped_options, '')

    def __serialize_extra_options(self) -> str:
        """ Serialize coach own options. """
        extra_options = self.extra_options
        zipped_extra_options = list(zip(extra_options.keys(), extra_options.values()))
        return reduce(lambda acc, option: acc + f"--{option[0]} {option[1]} ", zipped_extra_options, '')
